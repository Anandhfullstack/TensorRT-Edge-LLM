...
int main(int argc, char* argv[])
{
    ...
    if (profilerEnabled)
    {
        ...
        // ============================================================
    // Persistent ASR interactive mode
    // Model/runtime is already loaded above and stays in memory.
    // ============================================================

    LOG_INFO("==========================================");
    LOG_INFO("Qwen3-ASR persistent mode READY");
    LOG_INFO("Enter a WAV file path to transcribe");
    LOG_INFO("Type 'exit' to stop");
    LOG_INFO("==========================================");
    ...
    while (true)
    {
        ...
        if (!std::getline(std::cin, audioPath))
        {
            break;
        }

        if (audioPath == "exit" || audioPath == "quit")
        {
            LOG_INFO("Stopping persistent ASR...");
            break;
        }
        ...
        {
            std::cerr << "Audio file does not exist: "
                      << audioPath << std::endl;
            continue;
        }

        // --------------------------------------------------------
        // Create temporary request JSON using the same format
        // already supported by TensorRT-Edge-LLM.
        // --------------------------------------------------------
        ...
        requestJson["top_p"] = 1.0;
        requestJson["top_k"] = 50;
        requestJson["max_generate_length"] = 256;
        ...
        Json audioContent = Json::array();

        audioContent.push_back({
            {"type", "audio"},
            {"audio", audioPath}
        });
        ...
        requestJson["requests"] = Json::array();

        requestJson["requests"].push_back({
            {"messages", messages}
        });

        std::string const tempRequestPath
            = "/tmp/qwen3_asr_request.json";

        {
            std::ofstream tempFile(tempRequestPath);

            if (!tempFile.is_open())
            {
                std::cerr
                    << "Failed to create temporary request file"
                    << std::endl;
                continue;
            }

            tempFile << requestJson.dump(2);
        }

        // --------------------------------------------------------
        // Parse the new audio request.
        // IMPORTANT: this does NOT recreate the model runtime.
        // --------------------------------------------------------

        std::unordered_map<std::string, std::string> requestLoraMap;

        std::vector<rt::LLMGenerationRequest> requests;

        try
        {
            std::tie(requestLoraMap, requests)
                = parseInputFile(
                    tempRequestPath,
                    -1,
                    -1,
        ...
        if (requests.empty())
        {
            std::cerr << "No valid ASR request created."
                      << std::endl;
            continue;
        }

        // --------------------------------------------------------
        // Reuse the EXISTING TensorRT runtime
        // --------------------------------------------------------
        ...
        if (!success)
        {
            std::cerr << "ASR inference failed." << std::endl;
            continue;
        }
        ...
        {
            std::cout << "\nTranscription:\n"
                      << response.outputTexts[0]
                      << std::endl;
        }
        else
        {
            std::cerr << "No transcription returned."
                      << std::endl;
        }
    }
